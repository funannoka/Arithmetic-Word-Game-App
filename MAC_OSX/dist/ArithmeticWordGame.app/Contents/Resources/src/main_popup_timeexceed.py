...
class CntrlrCls():
    def __init__(self, **kwargs):
        ...
        #self.path = os.path.join(os.path.abspath("wordGame.kv")) 
        self.popon = 0
        self.debug = 1
        self.screenName = ""
...
class MyPopup(Popup):
    ...
    #def disable_entr_btn(self):
        #cntr.popon =1

    def _on_keyboard_down(self, instance, keyboard, keycode, text, modifiers):
        if keycode == 40 and cntr.popon==1:  # 40 - Enter key pressed 
            if(cntr.debug):
...
#     pass
#from kivy.resource import resource_add_path,resource_find
# def add_source_dep(Self):
#tools_path = os.path.dirname('ImageTheme.txt')
    #     icons_path = os.path.join(tools_path, 'NanumSquareL.ttfs')
    #     resource_add_path(icons_path)
#os.path.join(tools_path, 'words.txt')
#path = os.path.dirname(os.path.abspath(__file__))
#self.image = path + '/g3347.png'

class GameApp(App):
    #path = os.path.dirname(os.path.abspath("main.py"))#__file__
    frozen = 'not'
    ...
    icon = bundle_dir+'/images/one2oneMap.png'
    normThemeButton = "button"
    singleWordFileName = bundle_dir+"/words.txt"
    imageThemeFile = bundle_dir+'/ImageTheme.txt'#os.path.join(bundle_dir, 'ImageTheme.txt') #"/Users/Funa/data/src/ImageTheme.txt"
    workScreenName = "work"
    doneScreenName = "done"
    correctTag =  "Correct"
    ...
    def __init__(self, **kwargs):
        ...
        #self.t1 = threading.Thread(target=self.handlerT)
    ...
    def collectGridImages(self):
        ...
        dirRandTheme = self.bundle_dir+'/images/'+randTheme+'/'
    ...
    def randSentenceGenerator(self):
        randVerb = self.random_line(self.bundle_dir+'/words/verb.txt')
        randNoun = self.random_line(self.bundle_dir+'/words/'+randVerb+'-nouns.txt')
        randSubject = self.random_line(self.bundle_dir+'/words/subject.txt')
        randDeterminer = self.random_line(self.bundle_dir+'/words/determiner.txt')
    ...
    def collectWrdsRandSentenceGen(self):
        ...
        randSubList = []
        sentenceWrdArr = []
        randtheme = self.random_line(self.bundle_dir+'/words/theme.txt')
        if (self.gridsize > 9):
            cnt = 6
            cnt2 = 2
            ...
            cnt2 = 1
        for i in range (cnt):
            randVerbList.append(self.random_line_no_rep(self.bundle_dir+'/words/'+randtheme+'-verbs.txt',randVerbList))
            randNounList.append(self.random_line_no_rep(self.bundle_dir+'/words/'+randtheme+'-nouns.txt',randNounList))
            self.wrdDict[randVerbList[i]] = self.sStruct[1] 
            self.wrdDict[randNounList[i]] = self.sStruct[3]  
        for i in range (cnt2):
            randDetList.append(self.random_line_no_rep(self.bundle_dir+'/words/determiner.txt',randDetList))
            randSubList.append(self.random_line_no_rep(self.bundle_dir+'/words/subject.txt',randSubList))
    ...
    def collectOneQuestion (self):
        ...
        if self.firstQuest == 1: #if self.currGrid == 1 and self.qpos == 0:
            ...
            self.firstQuest =0
            self.stop_threads = False
            self.set_tick_interrupt(self.maxT)
    ...
    def handlerT(self): #,stop
        if(self.debug):
            print('running thread')
        time.sleep(self.tinterval2)
        #if stop(): #self.tExc
            #break
        ...
        #if ((time.time() - self.start)>self.tinterval2):
        else:
            self.timeExceed = 1
            #self.tExc = 0
            WorkingScreen().ids.entr.trigger_action(duration=0.1)
            #WorkingScreen().ids.entr.disabled = True
            #self.stop_threads = True
            #self.t1.join()

    def set_tick_interrupt(self,s):
        self.start = time.time()
        self.tinterval2 = s
        try:
            self.t1.join()
            ...
            self.t1 = threading.Thread(target=self.handlerT)
        #self.t1 = threading.Thread(target=self.handlerT, args =(lambda : self.stop_threads, ))

        self.t1.start() # start ticker in a new thread
        if(self.debug):
    ...
    def returnQuestion(self):
        ...
        self.start = time.time()
        #mixer.quit()
        return self.questionkvString,self.chngCntr(0.3) #self.textToSpeechgTTS(), 
    ...
    def handler(self):
        ...
        if(self.debug):
            print('cntr.popon turned off')
        cntr.popon = 0
        if(self.ansWord==False and self.endsq == 0):
            self.stop_threads = False
            self.set_tick_interrupt(self.maxT)
        #self.tinterval = time.time()
        #return self.done
    ...
    def collectAns(self):
        ...
        if (self.timeExceed):
            # if(self.ansWord):
            #     self.errMsg = self.timeExceedTag
    ...
    def reportSummary(self):
        ...
        if(self.mkSenten == True):
            ...
            if (self.qpos not in range(len(self.questionsList[self.currGrid-1]))):
                ...
                self.maxT = 30
                self.collectOneQuestion()
        WorkingScreen().ids.entr.disabled = False
            #self.textToSpeech()
    ...
    def build(self):
        #self.path = os.path.join(os.path.abspath("wordGame.kv"))  #self.user_data_dir# os.path.abspath(__file__)
        #icon = self.path+'/images/one2oneMap.png'
        #self.singleWordFileName = self.path+"/words.txt"
        #self.imageThemeFile = "ImageTheme.txt" #self.path+"/ImageTheme.txt"
        self.presentation = Builder.load_file("wordGame.kv")
        #Builder.load_files(os.path.join(dirname(__file__), '../view/app1.kv'))
